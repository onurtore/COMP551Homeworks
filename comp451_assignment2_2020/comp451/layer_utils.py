from comp451.layers import *
from comp451.fast_layers import *



def affine_lrelu_do_forward(x, w, b, lrelu_param,dropout,droput_param):

    do_cache  = None
    out,cache = affine_lrelu_forward(x, w, b,lrelu_param)
    if dropout:
        out, do_cache = dropout_forward(out,droput_param)

    return out,(cache[0],cache[1],do_cache)

def affine_lrelu_forward(x, w, b, lrelu_param):
    """
    Convenience layer that performs an affine transform followed by a LeakyReLU

    Inputs:
    - x: Input to the affine layer
    - w, b: Weights for the affine layer
    - lrelu_param: Leaky Relu parameter dict

    Returns a tuple of:
    - out: Output from the LeakyReLU
    - cache: Object to give to the backward pass
    """
    a, fc_cache = affine_forward(x, w, b)
    out, lrelu_cache = leaky_relu_forward(a, lrelu_param)
    cache = (fc_cache, lrelu_cache)
    return out, cache

def affine_lrelu_do_backward(dout, cache, dropout):

    fc_cache, lrelu_cache, do_cache  = cache
    if dropout:
        dout = dropout_backward(dout,do_cache)

    return affine_lrelu_backward(dout,(fc_cache,lrelu_cache))


def affine_lrelu_backward(dout, cache):
    """
    Backward pass for the affine-lrelu convenience layer
    """
    fc_cache, lrelu_cache = cache
    da = leaky_relu_backward(dout, lrelu_cache)
    dx, dw, db = affine_backward(da, fc_cache)
    return dx, dw, db


def affine_relu_forward(x, w, b):
    """
    Convenience layer that perorms an affine transform followed by a ReLU       

    Inputs:
    - x: Input to the affine layer
    - w, b: Weights for the affine layer

    Returns a tuple of:
    - out: Output from the ReLU
    - cache: Object to give to the backward pass
    """
    a, fc_cache = affine_forward(x, w, b)
    out, relu_cache = relu_forward(a)
    cache = (fc_cache, relu_cache)
    return out, cache


def affine_relu_backward(dout, cache):
    """
    Backward pass for the affine-relu convenience layer
    """
    fc_cache, relu_cache = cache
    da = relu_backward(dout, relu_cache)
    dx, dw, db = affine_backward(da, fc_cache)
    return dx, dw, db


def conv_lrelu_forward(x, w, b, conv_param, lrelu_param):
    """
    A convenience layer that performs a convolution followed by a LeakyReLU.

    Inputs:
    - x: Input to the convolutional layer
    - w, b, conv_param: Weights and parameters for the convolutional layer
    - lrelu_param: parameters for the leaky relu

    Returns a tuple of:
    - out: Output from the Leaky ReLU
    - cache: Object to give to the backward pass
    """
    a, conv_cache = conv_forward_fast(x, w, b, conv_param)
    out, lrelu_cache = leaky_relu_forward(a, lrelu_param)
    cache = (conv_cache, lrelu_cache)
    return out, cache


def conv_lrelu_backward(dout, cache):
    """
    Backward pass for the conv-lrelu convenience layer.
    """
    conv_cache, lrelu_cache = cache
    da = leaky_relu_backward(dout, lrelu_cache)
    dx, dw, db = conv_backward_fast(da, conv_cache)
    return dx, dw, db


def conv_relu_forward(x, w, b, conv_param):
    """
    A convenience layer that performs a convolution followed by a ReLU.

    Inputs:
    - x: Input to the convolutional layer
    - w, b, conv_param: Weights and parameters for the convolutional layer

    Returns a tuple of:
    - out: Output from the ReLU
    - cache: Object to give to the backward pass
    """
    a, conv_cache = conv_forward_fast(x, w, b, conv_param)
    out, relu_cache = relu_forward(a)
    cache = (conv_cache, relu_cache)
    return out, cache


def conv_relu_backward(dout, cache):
    """
    Backward pass for the conv-relu convenience layer.
    """
    conv_cache, relu_cache = cache
    da = relu_backward(dout, relu_cache)
    dx, dw, db = conv_backward_fast(da, conv_cache)
    return dx, dw, db


def conv_lrelu_pool_forward(x, w, b, conv_param, lrelu_param, pool_param):
    """
    Convenience layer that performs a convolution, a LeakyReLU, and a pool.

    Inputs:
    - x: Input to the convolutional layer
    - w, b, conv_param: Weights and parameters for the convolutional layer
    - lrelu_param: parameters of the LeakyReLU
    - pool_param: Parameters for the pooling layer

    Returns a tuple of:
    - out: Output from the pooling layer
    - cache: Object to give to the backward pass
    """
    a, conv_cache = conv_forward_fast(x, w, b, conv_param)
    s, lrelu_cache = leaky_relu_forward(a, lrelu_param)
    out, pool_cache = max_pool_forward_fast(s, pool_param)
    cache = (conv_cache, lrelu_cache, pool_cache)
    return out, cache


def conv_lrelu_pool_backward(dout, cache):
    """
    Backward pass for the conv-lrelu-pool convenience layer
    """
    conv_cache, lrelu_cache, pool_cache = cache
    ds = max_pool_backward_fast(dout, pool_cache)
    da = leaky_relu_backward(ds, lrelu_cache)
    dx, dw, db = conv_backward_fast(da, conv_cache)
    return dx, dw, db


def conv_relu_pool_forward(x, w, b, conv_param, pool_param):
    """
    Convenience layer that performs a convolution, a ReLU, and a pool.

    Inputs:
    - x: Input to the convolutional layer
    - w, b, conv_param: Weights and parameters for the convolutional layer
    - pool_param: Parameters for the pooling layer

    Returns a tuple of:
    - out: Output from the pooling layer
    - cache: Object to give to the backward pass
    """
    a, conv_cache = conv_forward_fast(x, w, b, conv_param)
    s, relu_cache = relu_forward(a)
    out, pool_cache = max_pool_forward_fast(s, pool_param)
    cache = (conv_cache, relu_cache, pool_cache)
    return out, cache


def conv_relu_pool_backward(dout, cache):
    """
    Backward pass for the conv-relu-pool convenience layer
    """
    conv_cache, relu_cache, pool_cache = cache
    ds = max_pool_backward_fast(dout, pool_cache)
    da = relu_backward(ds, relu_cache)
    dx, dw, db = conv_backward_fast(da, conv_cache)
    return dx, dw, db
