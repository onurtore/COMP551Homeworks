...
class FourLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        exp_s     = np.exp(scores)
        sum_exp_s = np.sum(exp_s,axis=1)[:,np.newaxis]
        ...
        """
        Matrix Sizes
        scores:TrainingSample*Classes (5,3)
        H3    :(5,10)
        W4    :(10,3)
        """

        dW4 = np.dot(H3_relu.T,probs)
        db4 = probs.sum(axis=0)


        #Result size should be 10,10
        #W4: (10,3)
        #H2: (5,10)
        #probs: (5,3) 
        #5,3 * 3,10 = 5,10
        ...
        dW3 = np.dot(probs,W4.T)
        dW3 = np.multiply(dW3, (H3>0))
        db3 = dW3.sum(axis=0)
        dW3 = np.dot(H2_relu.T,dW3)
        ...
        10,5  * (5,10)  = (5,10)
          (10,10)
        """
        dW2 = np.dot(probs, W4.T)
        
        dW2 = np.multiply(dW2,H3>0)
        
        dW2 = np.dot(dW2,W3.T)
        
        dW2 = np.multiply(dW2, H2>0)
        
        db2 = dW2.sum(axis=0)
        dW2 = np.dot(H1_relu.T, dW2)


        """
        Result size should be (4,10)
        W4   : (10,3)
        H1   : (5,10)
        probs: (5,3)
        W3   : (10,10)
        ...
        4,5 * (5,10)     = 4,10
        """
        dW1 = np.dot(probs, W4.T)
        dW1 = np.multiply(dW1,H3>0)
        dW1 = np.dot(dW1,W3.T)
        dW1 = np.multiply(dW1, H2>0)
        dW1 = np.dot(dW1,W2.T)
        dW1 = np.multiply(dW1,H1>0)
        db1 = dW1.sum(axis=0)
        dW1 = np.dot(X.T,dW1)

        #Take the regularizations
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            val_loss,val_grads = self.loss(X_val,y=y_val, reg=reg)
