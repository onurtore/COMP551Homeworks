...
class KNearestNeighbor(object):
    ...
    def compute_L2_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        XX_dot = np.sum(np.square(X),axis = 1)
        YY_dot = np.sum(np.square(self.X_train), axis = 1)
        XX_dot = XX_dot[:,np.newaxis]
        YY_dot = YY_dot[:,np.newaxis]
        #pshape([train_dot,test_dot])
        XY_dot = np.matmul(self.X_train,X.T).T
        dists  = XX_dot + YY_dot.T - (2 * XY_dot)
        dists  = np.sqrt(dists)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
    ...
    def compute_Cosine_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                ij_dot = np.matmul(self.X_train[j],X[i].T)
                i_L    = np.sqrt(np.sum(np.square(X[i])))
                j_L    = np.sqrt(np.sum(np.square(self.X_train[j])))
                dists[i, j] = ij_dot / (i_L * j_L)
    ...
    def compute_Cosine_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
                ...
                ij_dot = np.matmul(self.X_train,X[i].T)
                i_L    = np.sqrt(np.sum(np.square(X[i])))
                j_L    = np.sqrt(np.sum(np.square(self.X_train),axis=1))
                dists[i, :] = np.divide(ij_dot,np.multiply(i_L,j_L))
    ...
    def compute_Cosine_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ij_dot = np.matmul(self.X_train,X.T).T
        i_L    = np.sqrt(np.sum(np.square(X),axis=1))
        j_L    = np.sqrt(np.sum(np.square(self.X_train),axis=1))
        i_L    = i_L[:,np.newaxis]
        j_L    = j_L[:,np.newaxis]
        dists  = np.divide(ij_dot,np.multiply(i_L,j_L.T))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            label_count  = {i:closest_y.count(i) for i in closest_y}
            label_count  = {key:label_count[key] for key in sorted(label_count.keys())}
            min_val      = label_count[max(label_count.keys(), key=(lambda k: label_count[k]))]
            min_keys     = [i for i,j in label_count.items() if j == min_val]
            min_el       = min(min_keys)
            y_pred[i]    = min_el
